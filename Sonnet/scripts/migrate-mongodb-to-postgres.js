...
// ============================================================
// CONFIG — supply connection strings via the environment
// ============================================================

/**
 * Read a required env var, or exit loudly.
 *
 * SECURITY (2026-07-25): MONGO_URI and POSTGRES_URL below used to carry hardcoded
 * `process.env.X || '<literal credential>'` fallbacks. Committing this file leaked a
 * live MongoDB Atlas URI and a live Railway Postgres URL into a public repo; both had
 * to be rotated. The fallbacks were also a footgun independent of the leak — running
 * this script with no env set silently connected to PRODUCTION on both ends and would
 * have written to it. Never reintroduce a literal credential as a default.
 */
function requiredEnv(name, hint) {
  ...
  if (!value) {
    ...
    console.error(`         Set it in Sonnet/.env or inline:  ${name}=... node scripts/migrate-mongodb-to-postgres.js\n`);
    process.exit(1);
  }
  return value;
}
...
const CONFIG = {
  // MongoDB Atlas connection string (source — the retired gamecalenderbackend DB).
  MONGO_URI: requiredEnv(
    'MONGO_URI',
    'Source MongoDB connection string. The original Atlas `admin` user was deleted 2026-07-25 after the credential leak.'
  ),

  // PostgreSQL connection (destination).
  POSTGRES_URL: requiredEnv(
    'MIGRATION_DATABASE_URL',
    'Destination Postgres connection string. THIS SCRIPT WRITES DATA — point it at a LOCAL database unless you deliberately intend to modify production.'
  ),
