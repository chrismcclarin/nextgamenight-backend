...
//
// Phase 87.1 (BINT-02, Part B — D-02) — re-key EventRsvps onto the internal UUID
...
// rollback net) — it is DROP-COLUMN'd only in the D-08 follow-up PR, and removed
// from the Sequelize model only in Plan 09.
//
...
// models/EventRsvp.js is the sync()-built test-DB source. Both carry the FK.
...
//
// D-02 — ON DELETE CASCADE: a deleted user's RSVPs are removed. Orphan pre-clean
//   is a DELETE (faithful to CASCADE), logged, run BEFORE the ADD CONSTRAINT so the
//   ALTER cannot fail on pre-existing dirty data (T-87.1-02).
//
// T-87.1-01 — recreate the composite UNIQUE protection on the UUID column so the
//   one-RSVP-per-user-per-event invariant holds on the new key.
//
...
//   so a UUID-only RSVP write would hit a NOT NULL violation. Relaxing it to
...
const FK_NAME = 'eventrsvps_user_uuid_fkey';
...
module.exports = {
  async up(queryInterface) {
    ...
    await sequelize.transaction(async (t) => {
      ...
      const table = await queryInterface.describeTable('EventRsvps');
      if (!table.user_uuid) {
        await sequelize.query(
          `ALTER TABLE "EventRsvps" ADD COLUMN user_uuid UUID`,
          { transaction: t }
        );
        console.log('[RSVP-UUID] column user_uuid added.');
      } else {
        console.log('[RSVP-UUID] column user_uuid already present, skipping add.');
      }

      // (1) BACKFILL from Users (Auth0 string → UUID PK).
      const [, backfillMeta] = await sequelize.query(
        `UPDATE "EventRsvps" t SET user_uuid = u.id
           FROM "Users" u
          WHERE u.user_id = t.user_id
            AND t.user_uuid IS NULL`,
        { transaction: t }
      );
      console.log(`[RSVP-UUID] backfilled rows: ${backfillMeta ? backfillMeta.rowCount : 0}`);

      // (2) ORPHAN PRE-CLEAN — DELETE rows with no matching Users.id (CASCADE, D-02).
      const orphans = await sequelize.query(
        `DELETE FROM "EventRsvps" WHERE user_uuid IS NULL RETURNING id`,
        { type: QueryTypes.SELECT, transaction: t }
      );
      const deleted = Array.isArray(orphans) ? orphans.length : 0;
      console.log(`[RSVP-UUID] orphaned rows deleted: ${deleted}`);
      ...
      //     500 every EventRsvps INSERT during the deploy window, and it breaks the
      ...
      if (existing.length === 0) {
        ...
        console.log(`[RSVP-UUID] constraint ${FK_NAME} added (ON DELETE CASCADE).`);
      } else {
        console.log(`[RSVP-UUID] constraint ${FK_NAME} already present, skipping.`);
      }

      // (5) RECREATE INDEXES on the UUID column — composite UNIQUE (T-87.1-01,
      //     one RSVP per user per event) + standalone (user_uuid).
      await sequelize.query(
        `CREATE UNIQUE INDEX IF NOT EXISTS "eventrsvps_event_id_user_uuid_uq"
           ON "EventRsvps" (event_id, user_uuid)`,
        { transaction: t }
      );
      await sequelize.query(
        `CREATE INDEX IF NOT EXISTS "eventrsvps_user_uuid_idx"
           ON "EventRsvps" (user_uuid)`,
        { transaction: t }
      );

      // (6) RELAX RETAINED OLD COLUMN — MANDATORY (T-87.1-03). See header.
      await sequelize.query(
        `ALTER TABLE "EventRsvps" ALTER COLUMN "user_id" DROP NOT NULL`,
        { transaction: t }
      );
      console.log('[RSVP-UUID] old column user_id relaxed to nullable (DROP NOT NULL).');
    });
  },

  async down(queryInterface) {
    // Drops the new FK, new indexes, and new column only. Does NOT restore NOT NULL
    // on user_id — rows written after Plan 09 deploys may legitimately hold NULL.
    const sequelize = queryInterface.sequelize;
    await sequelize.query(
      `ALTER TABLE "EventRsvps" DROP CONSTRAINT IF EXISTS "${FK_NAME}"`
    );
    await sequelize.query(
      `DROP INDEX IF EXISTS "eventrsvps_event_id_user_uuid_uq"`
    );
    await sequelize.query(
      `DROP INDEX IF EXISTS "eventrsvps_user_uuid_idx"`
    );
    await sequelize.query(
      `ALTER TABLE "EventRsvps" DROP COLUMN IF EXISTS user_uuid`
    );
  },
};
